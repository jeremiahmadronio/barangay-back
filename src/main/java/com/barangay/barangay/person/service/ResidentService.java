...
public class ResidentService {
    ...
    public void updateResidentProfile(Long residentId, UpdateResidentProfileDTO dto, User actor, String ipAddress) {
        ...
        auditLogService.log(
                ...
                ipAddress,
                "Updated resident profile for: " + person.getFirstName() + " " + person.getLastName(),
                oldState,
    ...
    public void updateResidentStatus(Long residentId, UpdateStatusDTO dto, User actor, String ipAddress) {
        ...
        auditLogService.log(
                ...
                "STATUS_CHANGE",
                ipAddress,
                String.format("Changed status of %s %s from %s to %s. Reason: %s",
                        resident.getPerson().getFirstName(),
                        resident.getPerson().getLastName(),
                        oldStatus,
                        dto.status(),
                        dto.reason()),
                oldStatus.name(),
    ...
    @Transactional(readOnly = true)
    public ArchiveStatsDTO getArchiveDashboardStats() {
        ArchiveStatsDTO stats = residentRepository.getArchiveStatistics();
        if (stats == null) {
            return new ArchiveStatsDTO(0L, 0L, 0L, 0L);
        }
        return stats;
    }
