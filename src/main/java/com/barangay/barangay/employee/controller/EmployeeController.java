...
public class EmployeeController {
    ...
    public ResponseEntity<EmployeeStatsDTO> getGlobalEmployeeStats() {

        EmployeeStatsDTO stats = employeeService.getGlobalEmployeeStats();
    ...
    @GetMapping("/paged-table/global")
    public ResponseEntity<Page<EmployeeTableDTO>> getGlobalEmployees(
            @RequestParam(required = false) String search,
            @RequestParam(required = false) Long deptId,
            @RequestParam(required = false) Status status,
            @PageableDefault(size = 10, sort = "id", direction = Sort.Direction.DESC) Pageable pageable
    ) {
        Page<EmployeeTableDTO> result = employeeService.getGlobalPaginatedEmployees(
                search,
                deptId,
                status,
                pageable
