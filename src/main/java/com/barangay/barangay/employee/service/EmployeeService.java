...
public class EmployeeService {
   ...
   private final EmployeeCaseRepository caseRepository;
    ...
    private void logHireAudit(Employee emp, Person p, Department d, User officer, String ip) {
        try {
            ...
            auditLogService.log(
                    ...
                    "ASSIGN_OFFICER_MANAGEMENT",
                    Severity.INFO,
                    "ADD_OFFICER",
                    ip,
                    "add new officer: " + p.getFirstName() + " " + p.getLastName(),
                    null,
                    jsonState
            );
        } catch (Exception e) {
    ...
    public EmployeeStatsDTO getScopedEmployeeStats(UUID userId) {
        ...
        long inactive = employeeRepository.countByDepartmentIdInAndStatus(deptIds, Status.ACTIVE);
    ...
    public void updateEmployeeStatus(Long id, UpdateEmployeeStatus request, User officer, String ipAddress) {
        ...
        try {
            ...
            auditLogService.log(
                    ...
                    "UPDATE_STATUS",
                    ipAddress,
                    "Status update for " + savedEmployee.getPerson().getFirstName() + ": " + oldStatus + " to " + newStatus,
                    oldStatus,
                    newStatus
